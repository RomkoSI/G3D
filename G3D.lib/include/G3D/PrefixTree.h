...
namespace G3D {
    class PrefixTree : public std::enable_shared_from_this<PrefixTree> {
        // TODO friend class hierarchical menu. Then never expose non-root nodes ?
    protected:
        ...
        static Array<String> compactSplit(const String& s, char delimiter) {
            Array<String> result(stringSplit(s, delimiter));            
            rejectEmptyString(result);
            return result;
        }
                
        /** Note: modifies input array */
        static String compactJoin(Array<String>& elements, char delimiter) {
            rejectEmptyString(elements);
            return stringJoin(elements, delimiter);
        }
