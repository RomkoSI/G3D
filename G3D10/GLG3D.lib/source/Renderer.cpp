   ...
   (RenderDevice*                       rd,
    ...
    if (notNull(depthPeelFramebuffer)) {
        rd->pushState(depthPeelFramebuffer); {
            ...
            Surface::renderDepthOnly(rd, sortedVisibleSurfaces, CullFace::BACK, gbuffer->texture(GBuffer::Field::DEPTH_AND_STENCIL), depthPeelSeparationHint, Surface::TransparencyTestMode::REJECT_TRANSPARENCY, Color3::white() / 3.0f);
        } rd->popState();
    }
    END_PROFILER_EVENT();
}
