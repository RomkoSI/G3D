   ...
   (RenderDevice*                       rd,
    ...
    if (notNull(depthPeelFramebuffer)) {
        rd->pushState(depthPeelFramebuffer); {
            ...
            Surface::renderDepthOnly(rd, sortedVisibleSurfaces, CullFace::BACK, gbuffer->texture(GBuffer::Field::DEPTH_AND_STENCIL), depthPeelSeparationHint, TransparencyTestMode::REJECT_LESS_THAN_ONE, Color3::white() / 3.0f);
        } rd->popState();
    }
    END_PROFILER_EVENT();
}
