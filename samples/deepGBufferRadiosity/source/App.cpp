...
void App::onGraphics3D(RenderDevice* rd, Array<shared_ptr<Surface> >& all) {
    ...
    m_film->exposeAndRender(rd, filmSettings, m_framebuffer->texture(0), settings().hdrFramebuffer.depthGuardBandThickness.x, settings().hdrFramebuffer.depthGuardBandThickness.x);
}
